...
bool paralellelogram_intersect(Matrix3d Z, Vector3d pgram, Vector3d &intersection) {
    Vector3d X (pgram(0),pgram(1),pgram(2));
    intersection = Z.colPivHouseholderQr().solve(X);
    
    double a = intersection(0);
    double b = intersection(1);
...
void raytrace_parallelogram()
{
    ...
    for (unsigned i = 0; i < C.cols(); ++i)
    {
        for (unsigned j = 0; j < C.rows(); ++j)
        {
            ...
            const Vector3d dir = - ray_direction;
            Vector3d interesection;
            Matrix3d Z(3,3);
            Z << pgram_u, pgram_v, dir;

            // TODO: Check if the ray intersects with the parallelogram
            if (paralellelogram_intersect(Z,b,interesection))
            {
                // TODO: The ray hit the parallelogram, compute the exact intersection
                // point
                Vector3d ray_intersection = pgram_origin + interesection[0] * pgram_u + interesection[1] * pgram_v;
    ...
    for (unsigned i = 0; i < C.cols(); ++i)
    {
        for (unsigned j = 0; j < C.rows(); ++j)
        {
            ...
            if (ray_on_xy.norm() < sphere_radius)
            {
                ...
                const double specular = pow(diffuse,specular_exponent);
